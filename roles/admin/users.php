...
// AJAX request handling for adding a user
    ...
    <div class="max-w-7xl mx-auto space-y-6">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div class="lg:col-span-1 bg-white border border-slate-200 rounded-xl shadow-sm p-6">
                ...
                <form id="addUserForm" method="POST" class="space-y-5">
                    <div>
            ...
            if (validateForm()) {
                ...
                .then(data => {
                    if (data.success) {
                        const newRow = `
                            <tr class="hover:bg-slate-50 transition">
                                <td class="px-6 py-4 text-slate-800 font-medium">${data.user.full_name}</td>
                                <td class="px-6 py-4">
                                    <button onclick="openResetModal(${data.user.user_id})" class="text-blue-600 hover:text-blue-800 text-sm font-medium">Reset Password</button>
                                </td>
                                <td class="px-6 py-4 text-slate-700">${data.user.email}</td>
                                <td class="px-6 py-4">
                                    <span class="text-sm font-medium text-slate-700">${data.user.role_name}</span>
                                </td>
                                <td class="px-6 py-4 text-slate-600 text-sm">${data.user.created_at}</td>
                                <td class="px-6 py-4">
                                    <button onclick="openDeleteModal(${data.user.user_id})" class="text-red-600 hover:text-red-800 text-sm font-medium">Delete</button>
                                </td>
                            </tr>`;
                        document.getElementById('user-table-body').insertAdjacentHTML('afterbegin', newRow);
                        addUserForm.reset();
                        resetErrors();
